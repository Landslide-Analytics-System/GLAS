class DataExtractor:
	...
	def __init__(self, lid, file_name):
		self.lid = lid
	...
	def maxPrecip(self):
		...
		for i in range(len(self.data)):
			if self.data.startswith(search, i):
				res = max(res, float(self.data[i + len(search):self.data.find("</precipMM", i)]))
		return str(res)
	...
	def maxTemp(self):
		...
		for i in range(len(self.data)):
			if self.data.startswith(search, i):
				res = max(res, float(self.data[i + len(search):self.data.find("</HeatIndexF>", i)]))
		return str(res)
	...
	def maxAirPressure(self):
		...
		for i in range(len(self.data)):
			if self.data.startswith(search, i):
				res = max(res, float(self.data[i + len(search):self.data.find("</pressure>", i)]))
		return str(res)
	...
	def maxHumidity(self):
		...
		for i in range(len(self.data)):
			if self.data.startswith(search, i):
				res = max(res, float(self.data[i + len(search):self.data.find("</humidity>", i)]))
		return str(res)
	...
	def maxWind(self):
		...
		for i in range(len(self.data)):
			if self.data.startswith(search, i):
				res = max(res, float(self.data[i + len(search):self.data.find("</windspeedKmph>", i)]))
		return str(res)
